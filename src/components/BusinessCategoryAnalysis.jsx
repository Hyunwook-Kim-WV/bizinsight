...
// Industry Name Mapping (Fair Trade -> Seoul Data)
const NAME_MAPPING = {
    "한식": "한식음식점",
    "중식": "중식음식점",
    "일식": "일식음식점",
    ...
    "커피": "커피-음료",
    "편의점": "편의점",
    "종합소매점": "슈퍼마켓", // Mapping to Supermarket
    "화장품": "화장품",
    "이미용": "미용실",
    "네일": "네일숍",
    "피부": "피부관리실",
...
const IndustryRadar = ({ data, color, overlayData }) => {
    ...
    useEffect(() => {
        ...
        if (g.empty()) {
            ...
            for (let level = 0; level < levels; level++) {
                ...
                g.append("polygon")
                    ...
                    .attr("fill", "none")
                    .attr("stroke", "#94a3b8") // Darker grid
        ...
        console.log("Rendering IndustryRadar", { data, width, height });
        ...
        if (overlayData) {
            const overlayPath = g.selectAll(".overlay-path").data([overlayData.radarStats]);
            overlayPath.join(
                enter => enter.append("path")
                    .attr("class", "overlay-path")
                    .attr("d", line)
                    .attr("fill", "none")
                    .attr("stroke", "#fbbf24") // Amber-400 for overlay
                    .attr("stroke-width", 3)
                    .style("stroke-dasharray", "4 4")
                    .style("opacity", 1),
                update => update
            ...
            g.selectAll(".overlay-dot")
                .data(overlayData.radarStats)
                .join(
                    enter => enter.append("circle")
                        .attr("class", "overlay-dot")
                        .attr("cx", (d, i) => Math.cos(i * angleSlice - Math.PI / 2) * rScale(d.value))
                        .attr("cy", (d, i) => Math.sin(i * angleSlice - Math.PI / 2) * rScale(d.value))
                        .attr("r", 5)
                        .attr("fill", "#fbbf24")
            ...
            g.selectAll(".overlay-path").remove();
            g.selectAll(".overlay-dot").remove();
        }

    }, [data, color, overlayData]);

    return <svg ref={rRef} viewBox="0 0 300 300" width="100%" height="100%" className="overflow-visible"></svg>;
};

const EMPTY_ARRAY = [];

const BusinessCategoryAnalysis = ({ salesData = EMPTY_ARRAY, closureData = EMPTY_ARRAY }) => {
    ...
    const [startupCostMap, setStartupCostMap] = useState({}); // Dynamic Startup Cost Map
    ...
    useEffect(() => {
        // Only fetch if props are empty AND we haven't loaded internal data yet
        if (salesData.length === 0 && internalSalesData.length === 0) {
            const fetchData = async () => {
                ...
                try {
                    ...
                    const costCols = ['서비스_업종_코드_명', '합계금액'];
                    const costs = await loadCSV('공정위_2024년_창업비용.csv', 'EUC-KR', costCols);
                    console.log("Startup Cost data loaded:", costs.length, costs[0]);
                    ...
                    costs.forEach(row => {
                        ...
                        if (rawCost && rawName) {
                            const costVal = parseFloat(String(rawCost).replace(/,/g, ''));
                            if (!isNaN(costVal)) {
                                ...
                                if (mappedName) {
                                    costMap[mappedName] = costVal * 1000;
                                }
                            }
                        }
                    });
                    console.log("Startup Cost Map:", Object.keys(costMap).length, costMap);
    ...
    // Mock Startup Costs Removed - Using startupCostMap state
    ...
    useEffect(() => {
        ...
        const processed = Object.keys(industryStats).map(ind => {
            ...
            return {
                ...
                startupCost: startupCostMap[ind] || 0 // Default to 0 if missing
            };
        });

        // Filter out invalid (sales > 0, name not Unknown, and MUST have startup cost)
    ...
    useEffect(() => {
        ...
        const filtered = industryData.filter(d => {
            const meetsCost = d.startupCost >= minStartupCost * 100000000; // Slider is in 억 unit
        ...
        // Axes
        const xAxis = d3.axisBottom(xScale).tickFormat(d => (d / 100000000).toFixed(1) + "억");
    ...
    }, [industryData, minStartupCost, minSales, minSurvivalRate, selectedCategory, selectedIndustry]);
                    ...
                    <div className="col-span-12 lg:col-span-4 space-y-6">
                        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                            ...
                            <div className="space-y-6">
                                ...
                                <div>
                                    <div className="flex justify-between items-end mb-2">
                                        <label className="text-xs font-bold text-slate-500 uppercase">최소 생존율 (원 크기)</label>
                                        <span className="text-sm font-black text-blue-600">{minSurvivalRate}% 이상</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setMinSurvivalRate(Number(e.target.value))}
                                            className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600"
                                        />
                        ...
                        <div className="col-span-12 lg:col-span-9">
                            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                                <div className="flex justify-between items-center mb-6">
                                    <h4 className="font-bold text-slate-800 text-lg">주요 업종 비교 (Major Industry Comparison)</h4>
                                    {selectedIndustry && (
                                        <div className="flex items-center gap-2 bg-amber-50 px-3 py-1.5 rounded-lg border border-amber-100">
                                            <div className="w-2.5 h-2.5 rounded-full bg-amber-400"></div>
                                            <span className="text-xs font-bold text-amber-700">
                                                Comparing with: {selectedIndustry.name}
                                            </span>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                    {targetIndustries.map((target) => {
                                        const data = industryData.find(d => d.name === target.name);
                                        return (
                                            <div key={target.name} className="flex flex-col items-center">
                                                <h5 className="font-bold text-slate-700 mb-4">{target.label}</h5>
                                                {data ? (
                                                    <div className="w-full aspect-square max-w-[280px]">
                                                        <IndustryRadar data={data} color={target.color} overlayData={selectedIndustry} />
                                                    </div>
                                                ) : (
                                                    <div className="w-full aspect-square max-w-[280px] flex items-center justify-center bg-slate-50 rounded-full text-slate-400 text-xs">
                                                        Loading...
                                                    </div>
                                                )}
                                            </div>
